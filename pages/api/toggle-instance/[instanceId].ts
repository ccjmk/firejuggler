...
function startInstance(instance: InstanceConfig): number {
    console.log(`Starting instance ${instance.name} (id: ${instance.id})`);
    const fvtt = spawn('node', [`${instance.foundry.app}/resources/app/main.js`, `--dataPath=${instance.foundry.data}`], {
        detached: true,
    });
    return fvtt.id;
}
...
export default function handler(
    ...
    if (runningInstance) {
        console.log(`Found running instance.. stopping [${runningInstance.instance.name}]`);
